(function(){
  ...
  angular
    .module('scJobs')
    .controller('ScJobsCtrl', ScJobsCtrl);

  ScJobsCtrl.$inject = ['scUser', 'scJobs', 'scEmployers', 'scAlert', '$ionicLoading', '$scope', '$ionicModal', 'APP_GLOBALS'];

  /* @ngInject */
  function ScJobsCtrl(scUser, scJobs, scEmployers, scAlert, $ionicLoading, $scope, $ionicModal, APP_GLOBALS) {
    var vm = this;
    ...
    vm.getEmployersList = getEmployersList;
    ...
    function setJobsList() {
      ...
      $ionicModal.fromTemplateUrl( APP_GLOBALS.appModulesPath + APP_GLOBALS.appJobsModuleDir + 'sc.jobs-filters-modal.view.html', {
        scope: $scope,
    ...
    function getEmployersList() {
      ...
      vm.jobs.employers = [];
      scEmployers.getEmployersListByCenterRepId().then(function(data){
        if (data.status && data.status.success) {
          vm.jobs.employers = data.data;
          vm.jobs.employerId = vm.jobs.employers[0].id;
          vm.jobs.employerName = vm.jobs.employers[0].name;
          vm.getJobsList();
        } else {
          var msg = 'Error in getting employers list';
          if (data.status && data.status.msg) {
            msg = data.status.msg;
          }
          scAlert.error(msg);
        }
